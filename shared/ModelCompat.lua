...
---@param model integer The model hash
---@return table<string, boolean>? emoteSet A set of compatible emote names, or nil if compatible with all emotes
function GetCompatibleEmotes(model)
    -- Models not in the list are compatible with all emotes
    local compatGroup = models[model]
    if not compatGroup then
        return nil
    ...
    -- Collect all compatible emotes from this group and ancestors
    local compatibleEmotes = {}

    -- Add emotes from the current group
    if emoteCompatibilitySets[compatGroup] then
        for emoteName, _ in pairs(emoteCompatibilitySets[compatGroup]) do
            compatibleEmotes[emoteName] = true
        end
    end

    -- Add emotes from ancestor groups
    if ancestorMap[compatGroup] then
        for i = 1, #ancestorMap[compatGroup] do
            local ancestor = ancestorMap[compatGroup][i]
            if emoteCompatibilitySets[ancestor] then
                for emoteName, _ in pairs(emoteCompatibilitySets[ancestor]) do
                    compatibleEmotes[emoteName] = true
                end
            end
        end
